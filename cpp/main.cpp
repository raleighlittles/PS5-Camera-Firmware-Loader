...
int main(int argc, char *argv[])
{
    ...
    if (firmware_file.is_open())
    {
        ...
        for (uint32_t pos = 0; pos < length; pos += CHUNK_SIZE)
        {
            ...
            // Careful logging here, it'll slow things down enough to where the USB transfer won't complete in time
            // std::cout << "Transferred " << size << " bytes [" << pos << "," << (pos+size) << "], value=" << value << ", index=" << index << ", size=" << size << std::endl;

            if ((static_cast<uint32_t>(value) + size) > 0xFFFF)
            {
                index++;
            }
